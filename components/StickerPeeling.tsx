 ...
 * @framerDisableUnlink
 */
export default function StickerPeeling(__props: StickerPeelingProps) {
    const {
        image,
        imageWidth = 200,
        imageHeight = 200,
    ...
    useEffect(() => {
        if (!sceneRef.current || !hasContent) return
        const lastSize = lastBuiltSizeRef.current
        if (!lastSize || lastSize.width !== imageWidth || lastSize.height !== imageHeight) {
            recreateMeshForSize(imageWidth, imageHeight)
        }
    }, [imageWidth, imageHeight, hasContent, recreateMeshForSize])
    ...
    }, [resolvedImageUrl, setupScene, loadTexture, cleanupScene])
    ...
    // Back color update
    useEffect(() => {
        if (!meshRef.current || !loadedImageRef.current) return
        const img = loadedImageRef.current
        const materials = meshRef.current.material as any[]
        const backColorRgba = parseColorToRgba(resolveTokenColor(backColor))
